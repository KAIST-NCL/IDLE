...
class op_tf_sgd(LayerOperation):
    ...
    def run_time_operation(self, learning_option, cluster):
        """
        define stochastic gradient descent optimizer given loss and weights/biases
        outputs:
            output: training op
        ...
        def apiConstructor():
            ...
            sgd = tf.train.GradientDescentOptimizer(lr_method)
            if clip_grad is not None:
                grads = sgd.compute_gradients(loss, var_list=opt_vars)
                clipped_grads = [(tf.clip_by_value(grad, -1.0*clip_grad, 1.0*clip_grad), var) for grad, var in grads]
                train_op = sgd.apply_gradients(clipped_grads, global_step=global_step)
            else:
                train_op = sgd.minimize(loss, global_step=global_step, var_list=opt_vars)
