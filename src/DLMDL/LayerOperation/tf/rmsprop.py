...
class op_tf_rmsprop(LayerOperation):
    ...
    def run_time_operation(self, learning_option, cluster):
        ...
        eps = float(self.get_attr('epsilon', default=10**-10))
        ...
        def apiConstructor():
            ...
            rmsprop = tf.train.RMSPropOptimizer(lr_method, decay=decay, momentum=mom,
                                                epsilon=eps, centered=centered)
            if clip_grad is not None:
                grads = rmsprop.compute_gradients(loss, var_list=opt_vars)
