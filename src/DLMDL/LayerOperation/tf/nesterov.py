...
class op_tf_nesterov(LayerOperation):
    ...
    def run_time_operation(self, learning_option, cluster):
        ...
        def apiConstructor():
            ...
            nestrov = tf.train.MomentumOptimizer(lr_method, mom, use_nesterov=True)
            if clip_grad is not None:
                grads = nestrov.compute_gradients(loss, var_list=opt_vars)
                clipped_grads = [(tf.clip_by_value(grad, -1.0*clip_grad, 1.0*clip_grad), var) for grad, var in grads]
                train_op = nestrov.apply_gradients(clipped_grads, global_step=global_step)
            else:
                train_op = nestrov.minimize(loss, global_step=global_step, var_list=opt_vars)
