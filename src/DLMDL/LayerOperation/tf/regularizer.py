...
def get_regularizer(regularizer, is_bias=False):
    ...
    if type == 'L2':
        reg = tf.contrib.layers.l2_regularizer(scale=decay)
    elif type == 'L1':
        reg = tf.contrib.layers.l1_regularizer(scale=decay)
    elif type == 'L1_L2':
        reg = tf.contrib.layers.l1_l2_regularizer(scale_l1=decay, scale_12=decay)
